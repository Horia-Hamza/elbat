...
export const ForgotPasswordPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage(response?.message || 'إذا كان البريد الإلكتروني مسجلاً، فقد تم إرسال رابط إعادة التعيين.');
      setEmail('');
    } catch (err: any) {
      setError(err.message || 'حدث خطأ أثناء إرسال الرابط. يرجى المحاولة لاحقاً.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ fontSize: '1.6rem', fontWeight: 900, color: 'var(--primary-dark)', marginBottom: '0.5rem' }}>استعادة كلمة المرور</h2>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.88rem', marginBottom: '2rem' }}>أدخل بريدك الإلكتروني وسنرسل لك رابطاً لإعادة تعيين كلمة المرور.</p>
        ...
        {successMessage ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem', textAlign: 'right' }}>
              ...
              >
                {loading ? (
                  <><Loader2 size={18} style={{ animation: 'spin 1s linear infinite' }} /> جاري الإرسال...</>
                ) : (
            ...
            <div style={{ marginTop: '2rem', display: 'flex', justifyContent: 'center', gap: '0.5rem', alignItems: 'center', fontSize: '0.88rem' }}>
              <Link to="/login" style={{ color: 'var(--primary)', fontWeight: 700, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '4px' }}>
