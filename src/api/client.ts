...
export async function apiFetch<T>(
  ...
  try {
    ...
    if (DEV) console.error(`❌ [NETWORK ERROR] ${method} ${url}`, networkErr);
    throw networkErr;
  }

  // ── Handle 401 Unauthorized (Expired / Invalid Token) ──────
  if (res.status === 401) {
    if (DEV) console.warn('🔒 [401 Unauthorized] Token expired or invalid. Clearing all localStorage.');
    localStorage.clear();
  ...
  try {
    ...
    if (DEV) console.error(`❌ [READ ERROR] ${method} ${url} — status ${res.status}`);
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const statusHint =
        ...
        : `HTTP ${res.status}`;
      if (DEV) console.error(`❌ [${res.status}] ${method} ${url} — non-JSON body:`, rawText.substring(0, 300));
      throw new Error(statusHint);
    }
    // Non-JSON but ok (rare) — return raw text
    if (DEV) console.warn(`⚠️ [${res.status}] ${path} — non-JSON ok response`);
  ...
  if (DEV) {
    const responseGroup = ok ? `✅ [${res.status}] ${path}` : `❌ [${res.status}] ${path}`;
